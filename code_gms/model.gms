...
$if not set infeas_el_nodal              $set infeas_el_nodal true
$if not set infeas_heat                  $set infeas_heat true
$if not set infeas_lines                 $set infeas_lines true
...
*INFEAS_LINES.up(t,cb) = 10000;
*INFEAS_H_POS.up(t,ha) = 10000;
*INFEAS_H_NEG.up(t,ha) = 10000;

$ifthen %infeas_el_nodal% == True
...
Obj..                                            COST =e= sum((t,p), G(t,p)* mc(p))
                                                          + sum((t,p), H(t,p)*mc(p))
                                                          + sum((t,n), (INFEAS_EL_N_POS(t,n) + INFEAS_EL_N_NEG(t,n))*1E2)
                                                          + sum((t,ha), (INFEAS_H_POS(t,ha) + INFEAS_H_NEG(t,ha))*1E3)
                                                          + sum((t,cb), INFEAS_LINES(t,cb)*1E3)
                                                          + sum((t,z,zz), EX(t,z,zz)*1)
;
...
;
Gen_Max_CHP1(t,chp)..                            G(t,chp) =g= (g_max(chp)*0.85)/ h_max(chp) * H(t,chp)
;
Gen_Max_CHP2(t,chp)..                            G(t,chp) =l= g_max(chp)*(1-(0.15*H(t,chp)/h_max(chp)))
;
Gen_Max_RES(t,p)$(ts(p) and co(p))..             G(t,p)  =l= g_max(p) * ava(t,p)
;
Gen_Min_RES(t,p)$(ts(p) and co(p))..                  G(t,p)  =g= g_max(p) * ava(t,p) * 0.0
;
Gen_Max_RES_h(t,ts)$(h_max(ts))..                H(t,ts)  =l= h_max(ts) * ava(t,ts)
;
Gen_Min_RES_h(t,ts)$(h_max(ts))..                H(t,ts)  =g= h_max(ts) * ava(t,ts) * 0.0
;
...
;
STOR_EL_End(t_end,es)..                  L_es(t_end,es) =g= es_cap(es)*0.25
;

STOR_H(t,hs)..                           L_hs(t,hs) =e= L_hs(t,hs-1)$(ord(t)>1)
                                                        - H(t,hs)
                                                        + eta(hs)*D_hs(t,hs)
*+ hs_cap(hs)*0.25$(ord(t) eq 1)
;
STOR_H_Cap(t,hs)..                       L_hs(t,hs) =l= hs_cap(hs)
;
STOR_H_Max(t,hs)..                       D_hs(t,hs) =l= h_max(hs)
;
STOR_H_End(t_end,hs)..                   L_hs(t_end,hs) =g= hs_cap(hs)*0.25 + 1
;
