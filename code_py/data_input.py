...
class InputProcessing(object):
	...
	def process_d2cf_data(self):
		...
		self.data.net_export_nodes = pd.DataFrame(index=self.data.demand_el.index)

		for zone in self.data.zones.index:
			if zone in net_export_raw.from_zone.values:
				export_timeseries = net_export_raw[net_export_raw.from_zone == zone].groupby("timestep").sum()
			else:
				export_timeseries = pd.DataFrame(0, index=self.data.demand_el.index, columns=["export"])

			if zone in net_export_raw.to_zone.values:
				import_timeseries = net_export_raw[net_export_raw.to_zone == zone].groupby("timestep").sum()
			else:
				import_timeseries =  - pd.DataFrame(0, index=self.data.demand_el.index, columns=["export"])

			self.data.net_export[zone] = export_timeseries - import_timeseries
		...
		for zones in set([(from_zone,to_zone) for (from_zone,to_zone) in zip(net_export_raw.from_zone, net_export_raw.to_zone)]):
			...
			for n in nodes:
				self.data.net_export_nodes[n] = net_export_raw.export[(net_export_raw["from_zone"] == zones[0]) & \
															 (net_export_raw["to_zone"] == zones[1])].values/len(nodes)
		for n in list(set(self.data.nodes.index) - set(self.data.net_export_nodes.columns)):
			self.data.net_export_nodes[n] = 0

		self.data.demand_el -= self.data.net_export_nodes
