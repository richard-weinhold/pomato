...
def update_line_colors(option=0, range_start=0, range_end=100):
    ...
    if option == 0:
        ...
        for idx, loading in enumerate(steps):
            cond = abs(n_0_flows[timesteps])/lines.maxflow > loading/100
        ...
        for idx, loading in enumerate(steps):
            cond = abs(n_1_flows[timesteps]/lines.maxflow) > loading/100
