...
function add_variables_expressions!(pomato::POMATO)
	...
	@expression(model, COST_G,
	    sum(G[t, p]*data.plants[p].mc_el for p in 1:n.plants, t in n.t));
	@expression(model, COST_H,
		n.he > 0 ? sum(H[t, he]*data.plants[map.he[he]].mc_heat for he in 1:n.he, t in n.t) : 0);
...
function add_results!(pomato::POMATO, name::String)
	if JuMP.termination_status(pomato.model) == MOI.OPTIMAL
		pomato.results[name] = Results(pomato)
	else
...
function add_curtailment_constraints!(pomato::POMATO)
	...
	for node in filter(n -> size(data.nodes[n].res_plants, 1) > 0, 1:n.nodes)
		for t in 1:n.t
		 	add_to_expression!(RES_Node[t, node],  -sum(CURT[t, res] for res in data.nodes[node].res_plants))
	 	end
	 end
...
function redispatch_model!(pomato::POMATO, market_model_results::Dict, redispatch_zone::String)
	...
	if !(termination_status(pomato.model) == MOI.OPTIMAL)
	...
	# Clear Market Model and build Redispatch Model
	pomato.model =  Model(with_optimizer(Gurobi.Optimizer))
	model = pomato.model

	## Build Redispatch model
	@variable(model, G[1:n.t, redispatch_zone_plants] >= 0) # El. power generation per plant p
	@variable(model, EX[1:n.t, 1:n.zones, 1:n.zones] >= 0) # Commercial Exchanges between zones (row from, col to)
	@variable(model, INJ[1:n.t, 1:n.nodes]) # Net Injection at Node n
	@variable(model, F_DC[1:n.t, 1:n.dc]) # Flow in DC Line dc

	@variable(model, 0 <= INFEAS_EL_N_NEG[1:n.t, 1:n.nodes]) # <= 100*pomato.options["infeasibility"]["bound"])
	@variable(model, 0 <= INFEAS_EL_N_POS[1:n.t, 1:n.nodes]) # <= 100*pomato.options["infeasibility"]["bound"])
	...
	# for node in 1:n.nodes, t in n.t
	...
	# Expressions to create Nodal/Zonal Generation and Res Feedin
	@expression(model, G_Node[t=1:n.t, n=1:n.nodes], GenericAffExpr{Float64, VariableRef}(0));
	for p in 1:n.plants, t in 1:n.t
		add_to_expression!(G_Node[t, data.plants[p].node], p in redispatch_zone_plants ? G[t,p] : g_market[t, p])
	end
	...
	for ph in 1:n.ph, t in 1:n.t
		add_to_expression!(D_Node[t, data.plants[ph].node], d_ph_market[t, es])
	end
	...
	@expression(model, COST_EX, sum(EX)*1e-1);
	@expression(model, COST_INFEAS_EL, (sum(INFEAS_EL_N_POS) + sum(INFEAS_EL_N_NEG))*1e2);
	@expression(model, COST_CURT, GenericAffExpr{Float64, VariableRef}(0));
	...
	@objective(model, Min, model[:COST_G] + model[:COST_EX] + model[:COST_INFEAS_EL] + model[:COST_CURT]);
