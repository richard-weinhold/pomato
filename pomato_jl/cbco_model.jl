...
function m_ranges(m::Vector{Int})
	n = size(m, 1)
...
function parallel_matrix(A::Array{Float64}, b::Vector{Float64},
						 m::Vector{Int}, point::Vector{Float64},
						 ranges::Array{UnitRange{Int},1})

	tmp = zeros(length(m), 1)
	Threads.@threads for r in ranges
		@inbounds @fastmath tmp[r] = A[m[r],:]*point - b[m[r]]
	end
	return tmp
end

function RayShoot2(A::Array{Float64}, b::Vector{Float64}, m::Vector{Int},
	              z::Vector{Float64}, x_opt::Vector{Float64})
	### Set-up
	stepsize = 1
	i = 0
	...
	r = x_opt - z
	## Subset m to only contain constraints violated by x_opt
	m_hat = m[vec(parallel_matrix(A, b, m, x_opt,  m_ranges(m)) .> 0)]
	# m_hat = m
	if length(m_hat) == 0
		 @info("WARNING: m_hat empty, possibly numerical error")
		 @info("Moving further outside along the Ray")
		 m_hat = m[vec(parallel_matrix(A, b, m, x_opt*1.05,  m_ranges(m)) .> 0)]
		 @info("m_hat $(m_hat), length = $(length(m_hat))!")
	end
	ranges = m_ranges(m_hat)
	while true
		point = z + r*i
		temp = parallel_matrix(A, b, m_hat, point, ranges)

		# If 1 constraint is hit, return it
		constraints_hit = sum(temp .> 0)
		if constraints_hit == 1
			@debug("Constraint hit: $(m_hat[findall(x->x>0, temp)]) With i = $i, stepsize = $stepsize")
			return  m_hat[findfirst(x->x>0, temp)]
		# If more than one constraint is hit:
